...
for(i in 1:length(classifiers)){
  ...
  out <- capture.output(model$method)
  cat("Classifier", out, file=best_model_file, sep="\n", append=TRUE)
  out <- capture.output(model$bestTune)
  cat("Best parameter found:", out, file=best_model_file, sep="\n", append=TRUE)
  out <- capture.output(model$results)
  cat("Results:", out, file=best_model_file, sep="\n", append=TRUE)
